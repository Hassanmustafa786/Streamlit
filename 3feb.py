...
with st.sidebar:
    ...
    intro()
st.sidebar.write("Click for next Question")
# Trigger a rerun to update the UI
if st.sidebar.button("Next Question ▶️", key=f"{selected_language}"):
    st.rerun()
...
with tab1:
    ...
    if user_input:
        ...
        st.markdown(f'<div style="font-size: large; color: #F36D1F;"> {user_input}</div>', unsafe_allow_html=True)
        # st.write_stream(output)
        st.markdown(f'<div style="font-size: large; color: black;"> {output}</div>', unsafe_allow_html=True)
